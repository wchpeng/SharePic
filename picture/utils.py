...
def extend_albums_count(data, uid):
    ...
    picture_urls = Picture.objects.filter(id__in=first_picture_ids).only(
        "id", "picture"
    )
    ...
    if uid:
        ...
        favorite_albums = FavoriteAlbum.objects.filter(
            creater_id=uid, album_id__in=album_ids
        ).values("album_id")
    ...
    for d in data:
        ...
        d["first_picture"] = picture_id_map.get(
            d.get("first_picture_id"), "/static/base/bg_groups/green.jpg"
        )
...
def get_favorite_album_count(album_id):
    ...
    return get_count(
        ...
        value=album_id,
        obj=FavoriteAlbum,
        cache_key_format=FAVORITE_ALBUM_CACHE_KEY,
    )
...
def get_reply_album_count(album_id):
    ...
    return get_count(
        ...
        value=album_id,
        obj=Reply,
        cache_key_format=REPLY_ALBUM_CACHE_KEY,
    )
